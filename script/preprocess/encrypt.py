...
class Encryptor():
    ...
    def get_all_chars(self):
        if self.lang == 'latin':
            ...
            self.unk_symbol = chr(len(self.symbols))
        else:
    ...
    def encrypt_poly(self, text):
        ...
        for i in range(len(text)):
            ...
            # Map chars
            encrypted_text += self.symbols[(self.symbol2index[char] + shift) % len(self.symbols)] if char in self.symbol2index else self.unk_symbol
...
class EncryptPipeline(PipelineStep):
    def __init__(self, seed=0, type: Literal["poly", "chacha20"] = "poly", key_len: int = 1):
        ...
        if type == "poly":
            ...
            self.encryptor.chacha20(reuse_key=True, reuse_nonce=True)
        else:
            raise ValueError("Invalid encryption type. Choose either 'poly' or 'chacha20'.")
