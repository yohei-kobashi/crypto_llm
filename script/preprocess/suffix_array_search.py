#!/usr/bin/env python3
"""
Suffix‑array 50‑token matcher – *SentencePiece + Parquet/JSONL edition*
======================================================================
...
Supports both **.jsonl** and **.parquet** files under `--corpus` (extracting text
from the 'text' column in both formats).
...
Now with **progress bars** (tqdm) and **auto sep_id** detection.
...
Requires:
    pip install numpy sentencepiece pydivsufsort tqdm pyarrow
"""
...
SA_DTYPE           = np.uint64
LCP_DTYPE          = np.uint32
DEFAULT_SHARD_SIZE = 64_000_000            # tokens per shard (~1.3GB RAM)
...
def iter_token_ids(corpus_root: Path, sp: spm.SentencePieceProcessor, sep_id: int) -> Iterator[int]:
    jsonls = sorted(corpus_root.rglob("*.jsonl"))
    parqs   = sorted(corpus_root.rglob("*.parquet"))
    files = jsonls + parqs
            ...
            with file.open("r", encoding="utf-8", errors="ignore") as fh:
                for line in fh:
                    try:
                        ...
                        continue
                    for tid in sp.EncodeAsIds(text.strip()):
                        yield tid
        elif file.suffix == ".parquet":
            tbl = pq.read_table(str(file))
            col = tbl['text'] if 'text' in tbl.schema.names else tbl.column(0)
            for txt in col.to_pylist():
                for tid in sp.EncodeAsIds(str(txt).strip()):
                    yield tid
        # boundary token to prevent cross-file matches
        yield sep_id
...
def _build_sa_for_shard(token_file: Path) -> None:
    ...
    for i in range(len(tokens)):
        ...
        if k>0:
            j = sa[k-1]
            while i+h<len(tokens) and j+h<len(tokens) and tokens[i+h]==tokens[j+h]: h+=1
            lcp[k] = h
            if h: h-=1
    # Save via memmap for query-phase
...
def build_index(args):
    ...
    for tid in iter_token_ids(corpus, sp, sep_id):
        ...
        if len(buf)>=args.shard_size:
            ...
            shard_files.append(f); total+=n; shard_id+=1; buf=[]
    if buf:
        f,n=_write_shard(shard_id, buf, outdir)
        shard_files.append(f); total+=n
    ...
    with mp.Pool(args.workers) as pool:
        for _ in tqdm.tqdm(pool.imap_unordered(_build_sa_for_shard, shard_files),
                            total=len(shard_files), desc="Building SA", unit="shard"): pass
    # Prefix table
    ...
    with open(outdir/"prefix.tbl","wb") as fh: pickle.dump(tbl,fh)
...
def _sa_contains(tokens: np.ndarray, sa: np.ndarray, query: Sequence[int]) -> bool:
    lo,hi=0,len(sa); ql=len(query)
    while lo<hi:
        m=(lo+hi)//2
        slice_ = tokens[sa[m]:sa[m]+ql]
        if slice_.tolist()==list(query): return True
        if slice_.tolist()< list(query): lo=m+1
        else: hi=m
    return False
...
def query_index(args):
    ...
    sp = load_spm(args.spm_model)
    with open(outdir/"prefix.tbl","rb") as fh: tbl=pickle.load(fh)
    buckets = len(tbl)
    lines = Path(args.input).read_text(encoding="utf-8").splitlines()
    queries=[]
    for line in tqdm.tqdm(lines, desc="Tokenizing queries", unit="line"):
        ids=sp.EncodeAsIds(line.strip())
        if len(ids)>=MIN_MATCH: queries.append(ids[:MIN_MATCH])
    if not queries: print(f"No valid queries (need >= {MIN_MATCH} tokens)"); return
    tasks=[]
    for i,q in enumerate(queries):
        key=0
        for j in range(K_PREFIX): key=(key*31+q[j])&(buckets-1)
        for sid,_ in tbl[key]: tasks.append((i,sid,q,Path(args.index)))
    ...
    for i,ok in enumerate(found): print(f"Query {i}: {'MATCH' if ok else 'NO MATCH'}")
