...
class Encryptor():
    ...
    def get_all_chars(self):
        if self.lang == 'latin':
            ...
            self.symbols = [chr(code_point) for code_point in range(0x0021, 0x007F) if unicodedata.category(chr(code_point)) not in {'Cc', 'Cf', 'Cs', 'Co', 'Cn'}]
        elif self.lang == 'alpha':
            self.symbols = [chr(code_point) for code_point in list(range(0x0041, 0x005B)) + list(range(0x0061, 0x007B)) if unicodedata.category(chr(code_point)) not in {'Cc', 'Cf', 'Cs', 'Co', 'Cn'}]
        else:
        ...
        random.setstate(original_seed)
        self.symbol2index = {symbol:i+1 for i,symbol in enumerate(self.symbols)}
