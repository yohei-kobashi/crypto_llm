...
while [[ ${#} -gt 0 ]]; do
    case ${1} in
        ...
        --train_iters) train_iters=${2}; shift ;;
...
tensorboard_path="${output_model_dir}/tensorboard/${jobname}_${unique_id}"
...
wandb_options=" \
    ...
    --wandb_group pretrain_gpt_${model_size}B_${unique_id}"
if [[ -n "${wandb_tag}" ]]; then
